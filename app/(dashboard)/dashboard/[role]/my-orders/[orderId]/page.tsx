    ...
    return (
      <section className="app__admin-groceries">
          ...
          <div className="app__admin-dataList">
            {order.items.map((item) => (
              <article key={item.id} className="app__admin-dataRow">
                ...
                <div className="app__admin-dataStats">
                  <span>Line total: {formatCurrencyMinor(item.line_total_minor, item.currency)}</span>
                  {item.source_meal_id ? <span className="app__admin-tagPill">Sourced from meal plan</span> : null}
                </div>
              </article>
            ))}
          </div>
        ...
        <section className="app__admin-productSection">
          ...
          <ShopperStatusForm orderId={order.id} currentStatus={order.fulfillment_status} />
