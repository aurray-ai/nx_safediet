...
type MealOrderAssignFormProps = {
  orderId: string;
  currentWorkerId?: string | null;
  workers: WorkerSummary[];
};

export function MealOrderAssignForm({ orderId, currentWorkerId, workers }: MealOrderAssignFormProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!workerId) {
      setError("Choose a chef before assigning.");
      return;
    }
    setError(null);
    ...
    try {
      const response = await fetch(`/api/admin/meal-orders/${orderId}/assign`, {
        method: "POST",
  ...
  return (
    <form className="app__admin-stack" onSubmit={handleSubmit}>
      {currentWorkerId ? (
        <p className="app__admin-inlineMeta">Currently assigned — choose a different chef to reassign.</p>
      ) : (
        <p className="app__admin-inlineMeta">Currently unassigned — assign this order to begin preparation.</p>
      )}
